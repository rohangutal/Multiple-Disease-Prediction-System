...
# ── New modular imports ────────────────────────────────────────────────────
from symptom_engine import score_symptoms, get_symptom_list
from confidence_engine import enhance_confidence
...
def _train_diabetes_model():
    ...
    m.fit(Xtr, ytr, epochs=80, batch_size=32, validation_data=(Xv,yv),
          callbacks=[EarlyStopping(monitor='val_loss',patience=10,restore_best_weights=True,verbose=0)], verbose=0)
    m.save('diabetes_ann_model.h5')
    with open('diabetes_scaler.pkl','wb') as f: pickle.dump(sc, f)
    return m, sc


def _train_parkinson_model():
    csv = 'parkinsons.csv' if os.path.exists('parkinsons.csv') else 'parkinson.csv'
    if not os.path.exists(csv):
        return None, None
    print(f"[INFO] Training Parkinson ANN from {csv} ...")
    df = pd.read_csv(csv)
    ...
    m.save('parkinson_ann_model.h5')
    with open('parkinson_scaler.pkl','wb') as f: pickle.dump(sc, f)
    return m, sc
